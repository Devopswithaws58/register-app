pipeline{
    ...
    stages{
       ...
       stage('trigger CD Pipeline'){
        steps{
            script{
                sh "curl -v -k --user Cloud-User:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG:${IMAGE_TAG}' 'ec2-13-233-199-158.ap-south-1.compute.amazonaws.com:8080/job/GitOps-register-app-cd/buildWithParameters?token=gitops-token'"
                }
            }
       }
    }
